...
<form action="" id="form" class="my-10 w-3/12 m-auto left-2/4 top-2/4 p-3 bg-gray-200 flex flex-col border-2 rounded-md z-20">
    ...
    @error('name')
        <p class="text-red-500">Ошибка ввода имени{{$message}}</p>
    @enderror

    <input type="email" id="email" placeholder="Ваш email" required />
    @error('email')
        <p class="text-red-500">Ошибка ввода почты{{$message}}</p>
    @enderror
    ...
    />
    @error('telephone')
        <p class="text-red-500">Ошибка ввода телефона{{$message}}</p>
    @enderror
